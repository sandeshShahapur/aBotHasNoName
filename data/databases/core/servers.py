    ...
    async with db_pool.acquire() as connection:
        async with connection.transaction():
            prefix = await connection.fetchval(
                "SELECT prefix FROM servers where id = $1",
                message.guild.id
            )
            if prefix:
                return prefix
...
async def set_prefix(db_pool, server_id, prefix):
    async with db_pool.acquire() as connection:
        async with connection.transaction():
            await connection.execute(
                "INSERT INTO servers (id, prefix) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET prefix = $2",
                server_id, prefix
            )
...
async def set_default_role(db_pool, server_id, role_id):
    async with db_pool.acquire() as connection:
        async with connection.transaction():
            await connection.execute(
                "INSERT INTO servers (id, default_role) "
                "VALUES ($1, $2) "
                "ON CONFLICT (id) "
                "DO UPDATE SET default_role = $2",
                server_id, role_id
            )
