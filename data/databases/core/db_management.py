import discord
from data.databases.core.users import (
                            set_user,
                            set_server_user,
                            get_server_user
                        )
...
async def update_db(ctx, db_pool, server):
    ...
    await ctx.send('Loading users and their roles')
    # TODO potential data inconsistency where the bot is removed from the server and a member
    # TODO when removed a role, the bot would not be able to remove the role from the database.
    ...
    await ctx.send('Users and their roles loaded')

    await ctx.send('Loding plugins and their subplugins')
    ...
    await ctx.send('Plugins and their subplugins loaded')
...
async def sync_user_roles(db_pool, user: Optional[discord.Member], validate=False):
    ...
    # TODO after moderation plugin is implemented, remove the hardcoded role ids
    if 1079122386995122236 in cur_roles or 971526423112192061 in cur_roles or 953341678746480690 in cur_roles:
        return 0
    count = 0
